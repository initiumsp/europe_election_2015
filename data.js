var electionData = [
    {
    "area": "Croatia",
    "boundaryGeojsonPath": 'json/Croatia.json',
    "infoWindowAnchor": [45.81, 15.98],
    "election_results": [
        {
            "month": 1,
            "election_type": "presidential",
            "party_name_eng": "Croatian Democratic Union",
            "party_name_chi": "",
            "political_wing": "middle_right",
            "vote_rate": "50.74%"
        },
        {
            "month": 1,
            "election_type": "presidential",
            "party_name_eng": "Social Democratic Party of Croatia (SDP)",
            "party_name_chi": "",
            "political_wing": "middle_left",
            "vote_rate": "49.26%"
        },
    ]
    },
    {
        "area": "Italy",
        "boundaryGeojsonPath": 'json/Italy.json',
        "infoWindowAnchor": [41.90, 12.50],
        "electionResults": [{
            "month": 1,
            "election_type": "presidential",
            "party_name_eng": "Democratic Party",
            "party_name_chi": "",
            "political_wing": "middle_left",
            "vote_rate": "65.91%"
        },
        {
            "month": 1,
            "election_type": "presidential",
            "party_name_eng": "Five Star Movement",
            "party_name_chi": "",
            "political_wing": "",
            "vote_rate": "TBD"
        },]
    },
    {
        "area": "Greece",
        "boundaryGeojsonPath": 'json/Greece.json',
        "infoWindowAnchor": [37.96, 23.72],
        "electionResults": [
            {
                "month": 2,
                "election_type": "parliamentary",
                "party_name_eng": "SYRIZA",
                "party_name_chi": "",
                "political_wing": "far_left",
                "vote_rate": "36.30%"
            },
            {
                "month": 2,
                "election_type": "parliamentary",
                "party_name_eng": "New Democracy",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "27.80%"
            },
            {
                "month": 2,
                "election_type": "parliamentary",
                "party_name_eng": "Golden Dawn",
                "party_name_chi": "",
                "political_wing": "far_right",
                "vote_rate": "6.30%"
            },
            {
                "month": 2,
                "election_type": "parliamentary",
                "party_name_eng": "The River",
                "party_name_chi": "",
                "political_wing": "middle_left",
                "vote_rate": "6.10%"
            },
        ]
    },
    {
        "area": "Estonia",
        "boundaryGeojsonPath": "json/Estonia.json",
        "infoWindowAnchor": [59.44, 24.75],
        "electionResults": [
            {
                "month": 3,
                "election_type": "parliamentary",
                "party_name_eng": "Estonian Reform Party",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "27.70%"
            },
            {
                "month": 3,
                "election_type": "parliamentary",
                "party_name_eng": "Estonian Centre Party",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "24.80%"
            },
            {
                "month": 3,
                "election_type": "parliamentary",
                "party_name_eng": "Social Democratic Party",
                "party_name_chi": "",
                "political_wing": "middle_left",
                "vote_rate": "15.20%"
            },
            {
                "month": 3,
                "election_type": "parliamentary",
                "party_name_eng": "Pro Patria and Res Publica Union",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "13.70%"
            },
        ]
    },
    {
        "area": "France",
        "boundaryGeojsonPath": "json/France.json",
        "infoWindowAnchor": [48.86,  2.35],
        "electionResults": [
            {
                "month": 3,
                "election_type": "local",
                "party_name_eng": "Union for a Popular Movement (UMP)",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "29.00%"
            },
            {
                "month": 3,
                "election_type": "local",
                "party_name_eng": "National Front",
                "party_name_chi": "",
                "political_wing": "far_right",
                "vote_rate": "25.00%"
            },
            {
                "month": 3,
                "election_type": "local",
                "party_name_eng": "Socialist Party",
                "party_name_chi": "",
                "political_wing": "middle_left",
                "vote_rate": "13.30%"
            },
        ]
    },
    {
        "area": "Finland",
        "boundaryGeojsonPath": "json/Finland.json",
        "infoWindowAnchor": [60.17, 24.94],
        "electionResults": [
            {
                "month": 4,
                "election_type": "parliamentary",
                "party_name_eng": "Centre Party",
                "party_name_chi": "",
                "political_wing": "middle",
                "vote_rate": "21.10%"
            },
            {
                "month": 4,
                "election_type": "parliamentary",
                "party_name_eng": "Finns Party",
                "party_name_chi": "",
                "political_wing": "far_right",
                "vote_rate": "17.70%"
            },
            {
                "month": 4,
                "election_type": "parliamentary",
                "party_name_eng": "National Coalition",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "18.20%"
            },
            {
                "month": 4,
                "election_type": "parliamentary",
                "party_name_eng": "Social Democratic Party",
                "party_name_chi": "",
                "political_wing": "middle_left",
                "vote_rate": "16.50%"
            },
        ]
    },
    {
        "area": "Andorra",
        "boundaryGeojsonPath": "json/Andorra.json",
        "infoWindowAnchor": [42.50,  1.52],
        "electionResults": [
            {
                "month": 4,
                "election_type": "parliamentary",
                "party_name_eng": "Democrats for Andorra",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "37.03%"
            },
            {
                "month": 4,
                "election_type": "parliamentary",
                "party_name_eng": "Liberal Party of Andorra",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "27.68%"
            },
            {
                "month": 4,
                "election_type": "parliamentary",
                "party_name_eng": "Social Democratic, Green, Citizens' Initiative and independent candidates",
                "party_name_chi": "",
                "political_wing": "middle_left",
                "vote_rate": "23.53%"
            },
        ]
    },
    {
        "area": "UK",
        "boundaryGeojsonPath": "json/UK.json",
        "infoWindowAnchor": [51.51, -0.12],
        "electionResults": [
            {
                "month": 5,
                "country": "UK",
                "election_type": "parliamentary",
                "party_name_eng": "Conservative",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "36.90%"
            },
            {
                "month": 5,
                "country": "UK",
                "election_type": "parliamentary",
                "party_name_eng": "Labour",
                "party_name_chi": "",
                "political_wing": "middle_left",
                "vote_rate": "30.40%"
            },
            {
                "month": 5,
                "country": "UK",
                "election_type": "parliamentary",
                "party_name_eng": "UKIP",
                "party_name_chi": "",
                "political_wing": "far_right",
                "vote_rate": "12.60%"
            },
            {
                "month": 5,
                "country": "UK",
                "election_type": "parliamentary",
                "party_name_eng": "Liberal Democrats",
                "party_name_chi": "",
                "political_wing": "middle",
                "vote_rate": "7.90%"
            },
            {
                "month": 5,
                "country": "UK",
                "election_type": "parliamentary",
                "party_name_eng": "SNP",
                "party_name_chi": "",
                "political_wing": "middle_left",
                "vote_rate": "4.70%"
            },
        ]
    },
    {
        "area": "Poland",
        "boundaryGeojsonPath": "json/Poland.json",
        "infoWindowAnchor": [52.23, 21.02],
        "electionResults": [
            {
                "month": 5,
                "election_type": "presidential",
                "party_name_eng": "Law and Justice (PiS)",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "51.55%"
            },
            {
                "month": 5,
                "election_type": "presidential",
                "party_name_eng": "Civic Platform",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "48.45%"
            },
        ]
    },
    {
        "area": "Spain",
        "boundaryGeojsonPath": "json/Spain.json",
        "infoWindowAnchor": [40.40, -3.72],
        "electionResults": [
            {
                "month": 5,
                "country": "Spain",
                "party_name_eng": "People's Party",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "29.20%"
            },
            {
                "month": 5,
                "country": "Spain",
                "party_name_eng": "Spanish Socialist Workers' Party (PSOE)",
                "party_name_chi": "",
                "political_wing": "middle_left",
                "vote_rate": "24.10%"
            },
            {
                "month": 5,
                "election_type": "local",
                "party_name_eng": "Podemos",
                "party_name_chi": "",
                "political_wing": "far_left",
                "vote_rate": "13.50%"
            },
        ]
    },
    {
        "area": "Turkey",
        "boundaryGeojsonPath": "json/Turkey.json",
        "infoWindowAnchor": [41.01, 28.98],
        "electionResults": [
            {
                "month": 6,
                "election_type": "parliamentary",
                "party_name_eng": "Justice and Development Party",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "40.87%"
            },
            {
                "month": 6,
                "election_type": "parliamentary",
                "party_name_eng": "Republican People's Party",
                "party_name_chi": "",
                "political_wing": "middle_left",
                "vote_rate": "24.95%"
            },
            {
                "month": 6,
                "election_type": "parliamentary",
                "party_name_eng": "Nationalist Movement Party",
                "party_name_chi": "",
                "political_wing": "far_right",
                "vote_rate": "16.29%"
            },
            {
                "month": 6,
                "election_type": "parliamentary",
                "party_name_eng": "Peoples' Democratic Party",
                "party_name_chi": "",
                "political_wing": "middle_left",
                "vote_rate": "13.12%"
            },
        ]
    },
    {
        "area": "Denmark",
        "boundaryGeojsonPath": "json/Denmark.json",
        "infoWindowAnchor": [55.68, 12.57],
        "electionResults": [
            {
                "month": 6,
                "election_type": "parliamentary",
                "party_name_eng": "Social Democrats",
                "party_name_chi": "",
                "political_wing": "middle_left",
                "vote_rate": "26.30%"
            },
            {
                "month": 6,
                "election_type": "parliamentary",
                "party_name_eng": "Danish People's Party",
                "party_name_chi": "",
                "political_wing": "far_right",
                "vote_rate": "21.10%"
            },
            {
                "month": 6,
                "election_type": "parliamentary",
                "party_name_eng": "Venstre",
                "party_name_chi": "",
                "political_wing": "middle_right",
                "vote_rate": "19.50%"
            },
            {
                "month": 6,
                "election_type": "parliamentary",
                "party_name_eng": "Red–Green Alliance",
                "party_name_chi": "",
                "political_wing": "far_left",
                "vote_rate": "7.80%"
            }
        ]
    }
]
